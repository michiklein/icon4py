...
from stencils_combined import c2v2c_sum_program
...
values = xp.asarray(rng.random(grid.num_cells))
input_domain = gtx.domain({Dimension("Cell"): grid.num_cells})
output_domain = gtx.domain({Dimension("Cell"): grid.num_cells})
cell_input = gtx.as_field(input_domain, values, allocator=b_end)
cell_output = gtx.zeros(output_domain, allocator=b_end)
